...
uninstall:
	@cd ../installer; \
	helm uninstall cluster-connector --namespace=$(KUBE_NAMESPACE) || true
...
.PHONY: run
run:
	go run -mod=vendor cmd/cluster-connector/*.go run \
...
.PHONY: usage
usage:
	go run -mod=vendor ./cmd/cluster-connector usage \
